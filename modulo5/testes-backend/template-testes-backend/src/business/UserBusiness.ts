...
export class UserBusiness {
   ...
   public async getUserById(id:string, authorization:string) {
      try {
         if(!id || !authorization){
            throw new CustomError(400,"Os campos não foram preenchidos corretamente");  
         }

         const verifyId = tokenGenerator.verify(authorization)

         if(typeof verifyId !== "object"){
            throw new CustomError(400, "bad request");
         }

         const getUser = await userDatabase.getUserByEmail(id)
